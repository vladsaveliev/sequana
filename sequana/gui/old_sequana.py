...
class SequanaGUI(QW.QWidget):
    ...
    def initUI(self):
        ...
        self.preferences_dialog = PreferencesOptionsDialog(self)
        ...
        self.create_choice_button_sequana()
    ...
    def create_menu_bar(self):
        ...
        # preference Options
        prefAction = QW.QAction("Preferences", self)
        prefAction.setShortcut('Ctrl+P')
        prefAction.triggered.connect(self.preferences_dialog.exec_)
        ...
        options_menu.addAction(prefAction)
        if self._setup_ipython:
    ...
    def create_choice_button_sequana(self):
    ...
    def open_report(self):
        if self.pipeline_is_chosen and self.working_dir.get_filenames():
            ...
            browser = self.preferences_dialog._browser
...
class PreferencesOptionsDialog(QW.QDialog):
    def __init__(self, parent=None):
        super().__init__(parent=parent)
        self.main_layout = QW.QVBoxLayout(self)
        self.setWindowTitle("Preferences")

        settings = QtCore.QSettings("Sequana", "Preferences")
        self._browser = settings.value("browser", "pyqt5", type=str)
        self._htmlfile = settings.value("report filename", "multi_summary.html", type=str)
    ...
    def ok_event(self):
        settings = QtCore.QSettings("Sequana", "Preferences")
        settings.setValue("browser", self.browser_box.get_value())
        settings.setValue("htmlfile", self.html_box.get_value())
        self._browser = self.browser_box.get_value()
        self.close()

    def cancel_event(self):
        index = self.browser_box.combobox.findText(self._browser, QtCore.Qt.MatchFixedString)
        if index >= 0:
            self.browser_box.combobox.setCurrentIndex(index)
        self.close()
    ...
    def set_launch_options(self):
        self.browser_box = ComboBoxOption("Browser")
        self.browser_box.add_items(["pyqt5", "firefox", "safari", "chrome"])

        #self.html_box = TextOption("html file to open")

        general_widget = QW.QWidget()
        general_layout = QW.QVBoxLayout(general_widget)
        general_layout.addWidget(self.browser_box)
        #general_layout.addWidget(self.html_box)

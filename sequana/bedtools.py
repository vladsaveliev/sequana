...
class GenomeCov(object):
    ...
    def _read_csv(self, input_filename):
        ...
        with open(input_filename, "r") as fp:
            ...
            if not line.startswith("# sequana_coverage_analysis"):
                return None
            # get threshold, window size and circular info
            numblist = re_number.findall(line)
            thresholds = [float(f) for f in numblist[:4]]
            self.thresholds = DoubleThresholds(*thresholds)
            self.window_size = int(numblist[-1])
            circular = line.split(":")[-1]
            ...
            for line in fp:
                ...
                if chrom:
                    ...
                    break
            df = pd.read_csv(fp, header=None, names=line.split(","))
        chr_list = self._set_chr_list(df)
        # Set the link to this instance in each chromosome
        # useful if one wants to recompute GC content with different window
    ...
    def _set_chr_list(self, df):
        ...
        return [ChromosomeCov(df.loc[key], self.thresholds) for key in
                df.index.unique()]
    ...
    def to_csv(self, output_filename):
        ...
        with open(output_filename, "w") as fp:
            print("# sequana_coverage_analysis thresholds:%s window_size:%i "
                  "circular:%s" % (self.thresholds.get_args(),
                  self.window_size, self.circular), file=fp)
            for chrom in self.chr_list:
...
class ChromosomeCov(object):
    ...
    def running_median(self, n, circular=False):
        ...
        """
        if self.bed.window_size is None:
